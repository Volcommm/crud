...
function sortTable(columnIndex, thElement) {
    ...
    rows.sort((rowA, rowB) => {
        ...
        if (columnIndex >= 5) {
            ...
            // Gestione per testo (Numero Commessa, Data Apertura, ecc.)
...
function calculateTotals() {
    ...
    document.querySelectorAll('tbody tr').forEach(function(row) {
        if (!row.id.includes('totalsRow')) { // Escludi la riga dei totali
            let oreEffettive = parseFloat(row.cells[4].innerText.replace(/[^\d,.-]/g, '').replace(',', '.')) || 0;
    ...
    document.getElementById('totalOreEffettive').innerText = totalOreEffettive.toFixed(2).replace('.', ',');
